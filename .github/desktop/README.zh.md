...
应用源码、依赖版本和官方发布命令保持不变。此工作流使用官方构建、运行时准备、文件清单校验，以及打包后的 Host/Office 冒烟检查。这些检查不包含付费模型请求或交互式安装。
...
安装包不配置自动更新源或强制更新服务。后续社区版本需手动安装。Windows 使用 electron-builder 的标准 NSIS 安装器。macOS 应用 ID 和 `dsh` URL 处理器与上游一致；与其他 Harness 构建并存时，安装可能替换该应用或接管其 URL 处理器。
