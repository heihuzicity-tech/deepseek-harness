...
Application sources, dependency versions, and official release commands remain unchanged. This workflow uses the official build, runtime preparation, inventory verification, and packaged Host/Office smoke checks. These checks do not exercise paid model requests or interactive installation.
...
The installers do not configure an automatic update feed or mandatory-update service. Install later community versions manually. Windows uses electron-builder's standard NSIS installer. The macOS application ID and `dsh` URL handler match upstream; installing alongside another Harness build can replace that app or claim its URL handler.
